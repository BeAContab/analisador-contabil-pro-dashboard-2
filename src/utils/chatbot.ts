...
function answerWithoutReports(question: string): string {
  if (hasAny(question, ['privacidade', 'processamento local', 'seguranca'])) {
    return 'O produto foi desenhado para processar os PDFs localmente no navegador, sem upload do arquivo. O assistente de IA e opcional e vem desativado: so apos voce configurar sua chave da TESS e autorizar o aviso de privacidade um resumo pseudonimizado da analise passa a ser enviado a TESS.';
  }
