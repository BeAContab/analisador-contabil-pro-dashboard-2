...
| 💬 Assistente de IA | Integração com TESS ou modo local, sem dependência obrigatória de API |
...
```env
VITE_TESS_API_KEY=cole_sua_chave_aqui
```

A chave da TESS é opcional. Se o usuário informar a chave, o Workspace ID e o Agent ID no assistente, eles ficam salvos no `localStorage` do navegador daquele usuário e serão reaproveitados em acessos futuros no mesmo perfil do navegador.
...
src/
  ...
  utils/            Parsing, analises, exportacoes e integracao TESS
